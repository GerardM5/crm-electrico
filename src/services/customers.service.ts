...
export function useCustomers(filter: CustomersFilter = {}) {
  ...
  return useQuery<{ data: CustomerRow[]; count: number }>({
    ...
    queryFn: async () => {
      ...
      if (search) q = q.or(`name.ilike.%${search}%,company.ilike.%${search}%,email.ilike.%${search}%,city.ilike.%${search}%`)
...
export function useUpdateCustomer() {
  const qc = useQueryClient()
  return useMutation({
    mutationFn: async ({ id, ...payload }: UpdateDto<'customers'> & { id: string }) => {
      const { data, error } = await supabase.from('customers').update(payload as never).eq('id', id).select().single()
      if (error) throw error
      return data as CustomerRow
    },
    onSuccess: (_d, vars) => {
