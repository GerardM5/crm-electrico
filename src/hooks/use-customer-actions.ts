...
import { type CustomerRow, useUpdateCustomer } from '../services/customers.service'
...
export function useCustomerActions() {
      ...
      },
      {
        onSuccess: () => {
          logActivity.mutate({
            ...
            action: 'renewed',
            metadata: { label: `Contrato renovado hasta ${formatDate(renewalDate.toISOString())}` },
          })
  ...
  function logContact(input: { customerId: string; contactedAt: string; notes: string }) {
    updateCustomer.mutate(
      { id: input.customerId, last_contact_at: new Date(input.contactedAt).toISOString() },
      {
        onSuccess: () => {
          logActivity.mutate({
            entityType: 'customer',
            ...
            action: 'contacted',
            metadata: { label: `Llamada registrada: ${input.notes}` },
          })
  ...
  return { renewCustomer, logContact, isPending: updateCustomer.isPending }
}
