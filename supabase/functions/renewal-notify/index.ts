...
Deno.serve(async (req) => {
	...
	const { data: customers, error } = await supabase
		.from("customers")
		.select(
			"id, name, email, renewal_date, renewal_alert_months, assigned_to, profiles!customers_assigned_to_fkey(email, full_name)",
		)
		.is("deleted_at", null)
		.eq("status", "active")
	...
	for (const customer of customers ?? []) {
		...
		const daysLeft = Math.ceil(
			(new Date(renewalDate!).getTime() - today.getTime()) /
				(1000 * 60 * 60 * 24),
		);
		...
		console.log(
			`renewal-notify: ${customer.name} renews ${renewalDate} in ${daysLeft} days — notified ${assignee.email}`,
		);
	}

	// The customer status is now derived from contracts in the DB trigger.
